#!/usr/bin/env python3
"""
trim_scene_v9.py — Object-Trajectory Narratives (OTN) extractor, enhanced.

Compared to v8, this version keeps the same single-file, per-object CSV output
but adds a bit more temporal and interaction detail while staying compact and
LLM-friendly for smaller models.

New in v9 (on top of v8):
  - RegionTrace:
      Discrete, scene-relative region IDs along each path, downsampled.
      Encodes approximate spatial route without raw coordinates.
  - RegionDwellSummary:
      Top regions by dwell time (fraction of the object's active lifespan).
  - LifecycleEvents:
      Compact spawn/despawn markers per object.
  - InteractionTimeline:
      Lightweight per-object interaction events
      (near/touch segments with other important objects).

All of this is still compressed into a single trajectories.csv file, one row
per kept object, preserving v7/v8 columns for compatibility.
"""
...
# ---------- region mapping (scene-relative, x/z plane) ----------

def make_region_fn(all_x, all_z, grid_size=6):
    """
    Build a simple scene-relative region mapper on the X/Z plane.

    Regions are discrete IDs like 'R2_4' covering a grid over the scene's
    bounding box. This lets us express paths and events symbolically.
    """
    xs = np.array([v for v in all_x if np.isfinite(v)])
    zs = np.array([v for v in all_z if np.isfinite(v)])
    ...
    if not np.isfinite(range_x) or range_x <= 0:
        range_x = 1.0
    if not np.isfinite(range_z) or range_z <= 0:
        range_z = 1.0
...
# ---------- Region trace & dwell summary ----------

def build_region_trace(frames, px, pz, region_fn, max_points=16):
    """
    Downsampled sequence of region IDs along trajectory:
      "f0:R0_1;f20:R0_3;..."
    """
    valid = np.isfinite(px) & np.isfinite(pz)
    if valid.sum() == 0:
        return ""
    idx = np.where(valid)[0]
    if idx.size <= max_points:
    ...
    for i in sel:
        f = int(frames[i])
        reg = region_fn(float(px[i]), float(pz[i]))
        # avoid micro-oscillation noise in the string
        if reg == last_reg:
            continue
        parts.append(f"f{f}:{reg}")
        last_reg = reg
    return ";".join(parts)


def build_region_dwell_summary(frames, px, pz, region_fn, max_regions=4):
    """
    Approximate fraction of an object's active time spent in each region.
    Returns e.g. "R0_1:0.40;R0_2:0.35;R1_2:0.25"
    """
    valid = np.isfinite(px) & np.isfinite(pz)
    idx = np.where(valid)[0]
    if idx.size < 2:
    ...
    for i in range(idx.size - 1):
        ...
        if dt <= 0:
            continue
        reg = region_fn(float(px[i0]), float(pz[i0]))
        dwell[reg] += dt
    ...
    items = [(reg, dur / total) for reg, dur in dwell.items()]
    items.sort(key=lambda x: x[1], reverse=True)
    items = items[:max_regions]

    parts = [f"{reg}:{frac:.2f}" for reg, frac in items if frac > 0]
    return ";".join(parts)
...
# ---------- Object–object interactions (coarse + timeline) ----------

def _compress_interaction_events(events, max_segments=20):
    """
    events: list of (frame, rel, partner) for a single object.
    Returns a compact timeline string:
      "f10-20:near:E1;f45:touch:E2;..."
    """
    if not events:
        return ""

    # Sort by frame, then partner, then relation for stability
    events = sorted(events, key=lambda e: (e[0], e[2], e[1]))

    segments = []
    cur_start, cur_end, cur_rel, cur_partner = None, None, None, None

    for frame, rel, partner in events:
        frame = int(frame)
        if cur_start is None:
            cur_start, cur_end, cur_rel, cur_partner = frame, frame, rel, partner
            continue

        # Extend segment if same rel/partner and frames are contiguous or nearly so
        if rel == cur_rel and partner == cur_partner and frame <= cur_end + 1:
            cur_end = frame
        else:
            segments.append((cur_start, cur_end, cur_rel, cur_partner))
            cur_start, cur_end, cur_rel, cur_partner = frame, frame, rel, partner
    ...
    # Rank segments by duration (desc), then by start frame (asc), pick top-N
    ranked = []
    for s, e, rel, partner in segments:
        dur = max(1, e - s)
        ranked.append((dur, s, e, rel, partner))

    ranked.sort(key=lambda x: (-x[0], x[1]))
    top = ranked[:max_segments]
    # Re-sort selected by start time for readability
    top.sort(key=lambda x: x[1])

    parts = []
    for _, s, e, rel, partner in top:
        if e <= s:
            parts.append(f"f{s}:{rel}:{partner}")
        else:
            parts.append(f"f{s}-{e}:{rel}:{partner}")
    return ";".join(parts)
...
def compute_interaction_summaries(frames_df, object_names,
    ...
    For each pair of selected objects, we track:
      - minimum distance (for coarse summary)
      - per-frame near/touch events (for per-object timelines)

    Returns:
      summaries: dict ObjectName -> short interaction summary string.
      timelines: dict ObjectName -> compact interaction timeline string.
    """
    ...
    # Per-object raw interaction events (frame, rel, partner)
    raw_events = defaultdict(list)

    for frame, objs in frame_map.items():
        n = len(objs)
        for i in range(n):
            oi, x1, y1, z1 = objs[i]
            for j in range(i + 1, n):
                ...
                if rel is not None:
                    raw_events[oi].append((frame, rel, oj))
                    raw_events[oj].append((frame, rel, oi))

    # Build per-object textual summaries
    per_obj_summary = defaultdict(list)
    ...
    for (oi, oj), d in pair_min.items():
        ...
        def add(obj, other):
            if d <= touch_threshold:
                per_obj_summary[obj].append(f"touches {other}")
            elif d <= near_threshold:
                per_obj_summary[obj].append(f"moves close to {other}")
    ...
    summaries = {}
    for obj in names:
        events = per_obj_summary.get(obj, [])
        if not events:
            summaries[obj] = ""
            continue
    ...
    timelines = {}
    for obj in names:
        obj_events = raw_events.get(obj, [])
        timelines[obj] = _compress_interaction_events(
            obj_events,
...
def build_narrative_summary(row):
    ...
    region_trace = row.get("RegionTrace", "")
    region_dwell = row.get("RegionDwellSummary", "")
    ...
    # Region dwell + region trace
    if region_dwell:
        parts.append(f"It spends most of its time in regions: {region_dwell}.")
    if region_trace:
        parts.append("Key region transitions along its path: " + region_trace + ".")
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Generate trajectories_v9/trajectories.csv with enhanced OTN-Core + richer OTN-Text."
    )
...
def main():
    ...
    # Region mapper (based on all x/z positions)
    all_x = frames_df["pos_x"].to_numpy()
    all_z = frames_df["pos_z"].to_numpy()
    region_fn = make_region_fn(all_x, all_z, grid_size=6)
    ...
    for obj, g in frames_df.groupby("ObjectName"):
        ...
        # Temporal path trace (raw coordinates)
        path_trace = build_path_trace(frames, px, py, pz, max_points=12)

        # Region-based features
        region_trace = build_region_trace(frames, px, pz, region_fn, max_points=16)
        region_dwell_summary = build_region_dwell_summary(frames, px, pz, region_fn, max_regions=4)
        ...
        # Lifecycle events (very compact)
        lifecycle_events = f"spawn:f{start_frame};despawn:f{end_frame}"
        ...
        per_obj.append({
            ...
            "RegionTrace": region_trace,
            "RegionDwellSummary": region_dwell_summary,
    ...
    interaction_map, interaction_timeline_map = compute_interaction_summaries(
        frames_df,
    ...
    df["InteractionTimeline"] = df["ObjectName"].map(interaction_timeline_map).fillna("")
    ...
    rename_map = {
        ...
        "RegionTrace": "RegionTrace (Downsampled key region IDs along the trajectory)",
        "RegionDwellSummary": "RegionDwellSummary (Approximate fraction of time spent in each region)",
        "LifecycleEvents": "LifecycleEvents (Spawn/despawn markers in frame space)",
        ...
        "InteractionTimeline": "InteractionTimeline (Compact per-object timeline of near/touch segments)",
        ...
        "NarrativeSummary": "NarrativeSummary (Compact narrative description using motion, phases, regions & interactions)",
    }
    ...
    print(f"✅ Wrote {out_path} with {len(df)} enhanced OTN trajectories (v9).")
